...
cmd_run() {
  ...
  # 시크릿 마스킹
  # ------------------------------------------------------------
  # 기록은 커밋됩니다. 감싼 명령이 무엇을 출력할지 우리가 통제할 수 없으므로
  # 여기서 한 번 걸러야 합니다.
  #
  # 실제로 openshift-install 이 설치 완료 시 kubeadmin 비밀번호를 평문으로
  # 출력해서 기록에 그대로 들어간 적이 있습니다. 커밋 직전에 잡았습니다.
  # "안 나오겠지" 를 전제로 두면 언젠가 한 번은 들어갑니다.
  #
  # 홈 경로도 같이 지웁니다. 시크릿은 아니지만 기록은 공개 레포에 들어가고,
  # oc 나 openshift-install 의 오류 메시지는 파일 경로를 절대경로로 뱉습니다.
  # 남겨 봐야 읽는 사람에게 의미도 없습니다. 어차피 그 사람 홈은 다릅니다.
  sed -i.bak -E \
    -e "s|$HOME/|~/|g" \
    -e 's/(password: )"[^"]*"/\1"<redacted>"/g' \
    -e 's/(kubeadmin[^ ]* )[A-Za-z0-9]{5}-[A-Za-z0-9]{5}-[A-Za-z0-9]{5}-[A-Za-z0-9]{5}/\1<redacted>/g' \
    -e 's/[A-Za-z0-9]{5}-[A-Za-z0-9]{5}-[A-Za-z0-9]{5}-[A-Za-z0-9]{5}/<redacted>/g' \
    -e 's/(AKIA[0-9A-Z]{16})/<redacted>/g' \
    -e 's/("auth"[[:space:]]*:[[:space:]]*)"[^"]*"/\1"<redacted>"/g' \
    -e 's/(-----BEGIN [A-Z ]*PRIVATE KEY-----).*/\1<redacted>/g' \
    "$out" 2>/dev/null && rm -f "$out.bak"
