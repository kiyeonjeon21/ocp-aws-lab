      ...
      containers:
        - name: llama-server
          ...
          args:
            ...
            - -t
            - "4"
            # 동시 슬롯을 1개로 못박습니다. 기본값이 4 입니다.
            #
            # CPU 추론에서 이게 치명적입니다.
            # Open WebUI 는 사용자 메시지 하나에 LLM 호출을 여러 개 던집니다.
            # 본 응답 + 제목 생성 + 태그 생성 + 후속질문 생성이 동시에 갑니다.
            # 슬롯이 4개면 그 넷이 전부 받아들여져 스레드 4개를 나눠 쓰고,
            # 결과적으로 본 응답이 0.16 tok/s 까지 떨어집니다. 실측값입니다.
            #
            # 1개로 두면 나머지는 큐에서 기다립니다.
            # 전체 처리량은 같지만 사용자가 보는 첫 응답이 훨씬 빨라집니다.
            # GPU 로 옮기면 이 제약이 사라지므로 그때 되돌려도 됩니다.
            - --parallel
            - "1"
          ...
          resources:
            requests:
              ...
              memory: 3Gi
            limits:
              # 워커가 m6i.2xlarge(8 vCPU, allocatable 7500m)라 여유가 있습니다.
              # 3 -> 5 로 올리고 스레드를 4로 두면 생성 속도가 그만큼 붙습니다.
              # RHOAI 가 같은 노드에 올라오므로 더 올리지는 않습니다.
              cpu: "5"
              memory: 4Gi
          volumeMounts:
            - name: models
